...
def train_one_epoch(model, criterion, optimizer, scheduler, reader, hyper_params, device):
    ...
    # print(model)
    for x, y, action, delta, prop, _ in tqdm(reader):
        # Empty the gradients
        model.zero_grad()
        optimizer.zero_grad()
...
if __name__ == "__main__":
    ...
    if args.propensity_estimation is not None:
        torch.save(
            model_dict,
            f"models/{dataset}/propensity_estimator_{'deep' if not linear else 'linear'}{'_raw' if args.raw_image else ''}_{proportion}_tau{tau}.pth",
        )        
    elif args.train_ratio is not None:
        torch.save(
            model_dict,
            f"models/{dataset}/log_policy_{'deep' if not linear else 'linear'}{'_raw' if args.raw_image else ''}_{proportion}_tau{tau}_TrainRatio{args.train_ratio}.pth",
        )
    else:
        torch.save(
            model_dict,
            f"models/{dataset}/log_policy_{'deep' if not linear else 'linear'}{'_raw' if args.raw_image else ''}_{proportion}_tau{tau}.pth",
        )        
