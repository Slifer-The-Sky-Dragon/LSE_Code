...
def readfile(path):
    ...
    # )
    return x, action, prop, reward, labeled


class RecDataset2:
    def __init__(self, data_path, hyper_params):
        self.items = np.load(os.path.join(os.path.dirname(data_path), "item_features.npz"))["features"]
        # self.type = type
        (
            self.x,
            self.action,
    ...
    def collate_fn(self, data):
        x, action, prop, reward, labeled = zip(*data)
        x = torch.stack(x)
...
def load_data_fast2(hyper_params):
    store_folder = hyper_params.dataset["name"]
    path = f"../data/{store_folder}/bandit_data"
    path += "_sampled_" + str(hyper_params["num_sample"])
    bese_store_folder = store_folder.split("_")[0]
    base_path = f"../data/{bese_store_folder}/"
    train_dataset = RecDataset2(
        path + "_train.npz", hyper_params=hyper_params
    )
    train_loader = TorchDataLoader(
        train_dataset,
        num_workers=0,
    ...
    val_dataset = RecDataset2(path + "_val.npz", hyper_params=hyper_params)
    val_loader = TorchDataLoader(
        val_dataset,
        num_workers=0,
        batch_size=hyper_params["batch_size"],
    ...
    test_dataset = RecDataset2(path + "_test.npz", hyper_params=hyper_params)
    test_loader = TorchDataLoader(
        test_dataset,
        num_workers=0,
        batch_size=hyper_params["batch_size"],
